...
# Agreed JSON contract shape (worst-case: 6 spots)
SAMPLE_PAYLOAD = {
    "spot_1": "occupied",
    "spot_2": "free",
    "spot_3": "occupied",
    ...
    "fps": 28.30,
    "timestamp": "2026-04-18T01:32:01.123456+00:00",
}
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--log",
        default=None,
        help="Path to a CSV inference log to measure real payload sizes.",
    )
    p.add_argument(
        "--output",
        default=OUTPUT_FILE,
...
def measure_real_payloads(log_path: Path) -> list[int]:
    """Read a CSV log and estimate per-row JSON payload sizes in bytes."""
    sizes = []
    with open(log_path, newline="") as f:
        reader = csv.DictReader(f)
        for row in reader:
            # Reconstruct a representative JSON payload from the CSV row
